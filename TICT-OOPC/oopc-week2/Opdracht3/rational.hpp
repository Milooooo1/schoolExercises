...
public:
   ...
   /// operands are equal.
   bool operator==(const rational & rhs ) const {                             //TODO operator== does not reduce
   ...
   friend std::ostream & operator<<( std::ostream & lhs, const rational & rhs ){
      return lhs << std::hex <<"0x" << std::setfill('0') << std::setw(2) << rhs.counter << " , " << "0x" << std::setfill('0') << std::setw(2) << rhs.denominator;
   }   
   ...
   rational operator*( const rational & rhs ) const {
      rational x(counter * rhs.counter, denominator * rhs.denominator);
      return x;
   }
   ...
   /// \details
   /// This operator*= multilplies a rational with another rational
   /// and reduces it. 
   rational operator*=( const rational & rhs ){
